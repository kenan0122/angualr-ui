import { INavigationType, LinkConfig } from '../../../customHook';

class Link2 extends LinkConfig {
  preSetType: INavigationType = INavigationType.Link;
  css: string = this._getCss();

  readonly render = (
    leadingChar: string,
    text: string,
    link: string,
    title: string,
    ref: string,
    target: string
  ) => {
    return this._getHtmlTemplate(title, text);
  };

  createTemplate = (): string => {
    return `[01 标题内容](link-2 "https://cdn.ergolab.cn")`;
  };

  readonly getHtml = (): string => {
    return this._getHtmlTemplate('javascript:void(0)', '01标题内容');
  };

  private _getHtmlTemplate(title: string, text: string): string {
    return `<a href=${title}><section class="link2"><span class="link2-text bg-color"></span>${text}</section></a>`;
  }

  private _getCss(): string {
    return `
        .kingfar-wechat .link2 {
          padding: 5px 0;
        }
        .kingfar-wechat .link2-text{
          line-height: 1.5;
          font-size: 14px;
          display: inline-block;
          width: 8px;
          height: 8px;
          border-radius: 4px;
          margin-right: 10px;
          opacity: 0.8;
        }
        `;
  }
}

export const link_2 = new Link2();
