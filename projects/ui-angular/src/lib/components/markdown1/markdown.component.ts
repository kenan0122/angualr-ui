...
import { MdEditorOption } from './markdown.types';
...
export class MarkdownComponent implements ControlValueAccessor, Validator {
  ...
  @Input() public upload: Function | any;
  ...
  private get _hasUploadFunction(): boolean | undefined {
  ...
  ngOnInit() {
    ...
    // markedRender.image(this._getRender('image'), null, '');
    // markedRender.code(this._getRender('code'),false);
  ...
  onDrop(evt: DragEvent) {
    ...
    this._uploadFiles((evt.dataTransfer as any).file);
  }
  ...
  onAceEditorPaste(event: ClipboardEvent): void {
    if (event instanceof ClipboardEvent && (event.clipboardData as any).files.length > 0) {
      this._uploadFiles((event.clipboardData as any).files);
    }
  }
